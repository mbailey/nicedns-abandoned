module Client
  class Base
    ...
    class << self
      ...
      def find_single(id, params = {})
        ...
        if o[class_name.downcase]
          ...
          raise Client::Errors::NotFound, "Record not found"
        end
      end
      ...
      ## at the moment, nothing in Client needs to be pluralized with anything
      ...
      def create_object(attributes, objects = [])
        ...
        o.id         = attributes['id']
        for key, object in objects.select{|k,v| v.kind_of?(Client::Base)}
    ...
    ## a Client::Error. Optionally, pass a second 'data' parameter to send data to the post action.
